...
# Fetching remote state data from the admin configuration
...
# IAM policy for worker nodes to access SSM and EC2 describe instances
...
# IAM role for the Kubernetes master node
...
# IAM role for the Kubernetes worker nodes
...
# Attach worker policy to master role
...
# Instance profile for the worker nodes
...
# Auto Scaling policies for scaling in and out
