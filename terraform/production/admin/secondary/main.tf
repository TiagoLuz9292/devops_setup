...
data "terraform_remote_state" "networking" {
  ...
  config = {
    ...
    key            = "networking/vpc_secondary/terraform.tfstate"
    region         = "eu-north-1"
...
resource "aws_instance" "admin_secondary" {
  ...
  tags = {
    Name  = "Admin-Server-Secondary"
    Group = "Admin"
  }
}

resource "aws_eip" "admin_eip" {
  instance = aws_instance.admin_secondary.id
}
